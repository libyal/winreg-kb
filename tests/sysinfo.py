...
class SystemInfoCollectorTest(test_lib.BaseTestCase):
    ...
    def _CreateTestRegistry(self):
        ...
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "CurrentBuildNumber", data=value_data, data_type=dfwinreg_definitions.REG_SZ
        )
        registry_key.AddValue(registry_value)

        value_data = self._CURRENT_TYPE.encode("utf-16-le")
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "CurrentType", data=value_data, data_type=dfwinreg_definitions.REG_SZ
        )
        registry_key.AddValue(registry_value)

        value_data = self._CURRENT_VERSION.encode("utf-16-le")
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "CurrentVersion", data=value_data, data_type=dfwinreg_definitions.REG_SZ
        )
        registry_key.AddValue(registry_value)

        value_data = b"\x47\xc8\xda\x4c"
        registry_value = dfwinreg_fake.FakeWinRegistryValue(
            "InstallDate", data=value_data, data_type=dfwinreg_definitions.REG_DWORD
        )
        registry_key.AddValue(registry_value)
    ...
    def testCollect(self):
        ...
        collector_object = sysinfo.SystemInfoCollector(output_writer=test_output_writer)
        ...
        self.assertEqual(
            collector_object.system_information.current_version, self._CURRENT_VERSION
        )
