...
class TaskCacheDataParser(data_format.BinaryDataFormat):
    ...
    def ParseDynamicInfo(self, value_data, cached_task):
        ...
        if self._debug:
            self._output_writer.WriteValue("Unknown1", f"0x{dynamic_info.unknown1:08x}")
            ...
            self._output_writer.WriteValue("Unknown2", f"0x{dynamic_info.unknown2:08x}")

            self._output_writer.WriteValue("Unknown3", f"0x{dynamic_info.unknown3:08x}")
...
class TaskCacheCollector(interface.WindowsRegistryKeyCollector):
    ...
    def Collect(self, registry):  # pylint: disable=arguments-differ
        ...
        for subkey in tasks_key.GetSubkeys():
            ...
            if self._debug:
                if (
                    task_cache_key.last_written_time
                    and task_cache_key.last_written_time.timestamp
                ):
