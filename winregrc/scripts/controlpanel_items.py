...
def Main():
    ...
    argument_parser = argparse.ArgumentParser(
        description=("Extracts Windows control panel items from the Windows Registry.")
    )
    ...
    for source_definition in source_definitions:
        ...
        for item in collector_object.Collect(scanner.registry):
            ...
            existing_item = control_panel_item_per_identifier.get(item.identifier, None)

            # Ignore a module name that is the same as the identifier.
            if item.module_name and item.module_name.lower() == item.identifier:
            ...
            if not existing_item:
                ...
                item.module_name
                and item.module_name != existing_item.module_name
                and item.module_name not in existing_item.alternate_module_names
            ):
                existing_item.alternate_module_names.append(item.module_name)

            if item.identifier not in (windows_versions_per_control_panel_item):
    ...
    try:
        ...
        for identifier, windows_versions in sorted(
            ...
            output_writer_object.WriteKnownFolder(control_panel_item, windows_versions)
