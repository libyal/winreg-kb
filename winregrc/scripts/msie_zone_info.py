...
class StdoutWriter(output_writers.StdoutOutputWriter):
    ...
    def WriteZoneInformation(self, zone_information):
        ...
        if control_description:
            text = (
                f"Control\t\t\t: {zone_information.control:s} "
        ...
        if control_value_description:
            text = (
                f"Value\t\t\t: {zone_information.control_value!s} "
...
def Main():
    ...
    argument_parser = argparse.ArgumentParser(
        description=("Extracts the MSIE zone information from the Windows Registry.")
    )
    ...
    collector_object = msie_zone_info.MSIEZoneInformationCollector(debug=options.debug)
