...
class StdoutWriter(output_writers.StdoutOutputWriter):
    ...
    def WriteEventLogProvider(self, eventlog_provider):
        ...
        for index, path in enumerate(
            sorted((eventlog_provider.category_message_files))
        ):
            if index == 0:
        ...
        for index, path in enumerate(sorted((eventlog_provider.event_message_files))):
            if index == 0:
        ...
        for index, path in enumerate(
            sorted((eventlog_provider.parameter_message_files))
        ):
            if index == 0:
...
def Main():
    ...
    argument_parser = argparse.ArgumentParser(
        description=("Extracts Windows Event Log providers from the Windows Registry.")
    )
