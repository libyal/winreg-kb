...
class ProgramsCacheDataParser(object):
  ...
  def Parse(self, value_data):
    ...
    if self._debug:
      self._output_writer.WriteDebugData('Value data:', value_data)
    ...
    if header.format_version == 1:
      ...
      if self._debug:
        value_string = '0x{0:08x}'.format(header9.unknown2)
      ...
      if self._debug:
        ...
        self._output_writer.WriteValue('Known folder identifier', value_string)
    ...
    if header.format_version != 9:
      ...
      if self._debug:
        value_string = '0x{0:02x}'.format(sentinel)
        self._output_writer.WriteValue('Sentinel', value_string)
    ...
    while sentinel in (0, 1):
      ...
      if self._debug:
        ...
        self._output_writer.WriteValue('Entry data offset', value_string)

        self._output_writer.WriteIntegerValueAsDecimal(
            'Entry data size', entry_data_size)
      ...
      for shell_item in iter(shell_item_list.items):
        if self._debug:
          ...
          print('Shell item: {0:s}'.format(getattr(shell_item, 'name', '')))
    ...
    if value_data_offset < value_data_size:
      value_string = '0x{0:08x}'.format(value_data_offset)
      self._output_writer.WriteValue('Trailing data offset', value_string)
